...
// Don't swap rungs 1 and 2 when using tables, special case, used mostly by index encoding
...
0x5011, 0x5015, 0x5019, 0x6003, 0x501d, 0x6007, 0x600b, 0x600f, 0x6013, 0x6017, 0x601b, 0x601f, 0x6023, 0x6027, 0x602b, 0x602f,
...
0x6031, 0x7003, 0x7007, 0x700b, 0x6035, 0x6039, 0x603d, 0x700f, 0x7013, 0x7017, 0x701b, 0x701f, 0x7023, 0x7027, 0x702b, 0x702f,
...
0x7031, 0x7035, 0x7039, 0x703d, 0x7041, 0x7045, 0x7049, 0x704d, 0x7051, 0x7055, 0x7059, 0x705d, 0x7061, 0x8003, 0x8007, 0x800b,
0x800f, 0x8013, 0x8017, 0x801b, 0x7065, 0x7069, 0x706d, 0x7071, 0x7075, 0x7079, 0x707d, 0x801f, 0x8023, 0x8027, 0x802b, 0x802f,
...
0x80b1, 0x80b5, 0x80b9, 0x80bd, 0x80c1, 0x9003, 0x9007, 0x900b, 0x900f, 0x9013, 0x9017, 0x901b, 0x901f, 0x9023, 0x9027, 0x902b,
0x902f, 0x9033, 0x9037, 0x903b, 0x80c5, 0x80c9, 0x80cd, 0x80d1, 0x80d5, 0x80d9, 0x80dd, 0x80e1, 0x80e5, 0x80e9, 0x80ed, 0x80f1,
0x80f5, 0x80f9, 0x80fd, 0x903f, 0x9043, 0x9047, 0x904b, 0x904f, 0x9053, 0x9057, 0x905b, 0x905f, 0x9063, 0x9067, 0x906b, 0x906f,
...
static size_t ienc(const T grp[B2], size_t rung, size_t oldrung, oBits &s) {
    ...
    // Encode indices using the three bit table since len <= 8
    for (int i = 0; i < B2; i++) {
        // No middle inversion here
        static const uint8_t val[] = { 0, 2, 1, 5, 3, 7, 11, 15 };
        static const uint8_t len[] = { 2, 2, 3, 3, 4, 4,  4,  4 };
        int j = 0;
        while (v[j].value != grp[i])
            j++;
        acc |= uint64_t(val[j]) << abits;
        abits += len[j];
    }
    s.push(acc, abits);
